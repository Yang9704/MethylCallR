...
Outlier <- MeCall.Outlier(meth = data.norm$beta, pd = data.filtered$pd, group = "Group", nrep = 1000, sample.fraction = 0.9, conf = 0.999, n.component = 2,  plot=TRUE)
```
...
Remaineds <- setdiff(colnames(data.norm$beta), Outlier[[1]])
data.filtered$pd <- data.filtered$pd[Remaineds,]
data.norm$beta <- data.norm$beta[,Remaineds]
```
...
data.Heatmap <- MeCall.FindBatch(meth = data.norm$beta, pd = data.filtered$pd, batches = NULL, n.comp = 0)
```
...
data.combat <- MeCall.RemoveBatch(meth = data.norm$beta, pd = data.filtered$pd, batches=c("Sentrix_Position_original","Sentrix_ID_Original"), interest="Group", do.sva= FALSE, exception = NULL)
```
